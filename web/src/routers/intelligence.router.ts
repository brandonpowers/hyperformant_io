...
intelligenceApp.openapi({
  ...
  const [entity, indexDef] = await Promise.all([
    ...
    prisma.indexDefinition.findUnique({ where: { id: body.indexDefinitionId } }),
  ]);
...
intelligenceApp.openapi({
  ...
  const signals = body.includeSignals ? await prisma.signal.findMany({
    where: {
      timestamp: {
  ...
  const connections = body.includeConnections ? await prisma.connection.findMany({
    where: {
      OR: [
  ...
  const metrics = body.includeMetrics ? await prisma.metricPoint.findMany({
    where: {
      entityId: { in: body.entityIds },
      timestamp: {
        gte: new Date(timeRange.from),
  ...
  const analysisResults = entities.map(entity => {
    ...
    const entityConnections = connections.filter(c => 
      c.sourceEntityId === entity.id || c.targetEntityId === entity.id
    );
    ...
    // Risk assessment
    const negativeSignals = entitySignals.filter(s => 
    ...
    const competitorConnections = entityConnections.filter(c => 
      c.type === 'COMPETITOR'
    );
    ...
    return {
      ...
      scores: {
        ...
        overall: Math.round((activityScore + connectivityScore + growthScore) * 33.33) / 100,
      },
      insights,
      risks,
      opportunities,
