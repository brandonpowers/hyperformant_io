...
signalsApp.openapi(
  {
    ...
    description:
      'Retrieve signals with optional filtering by time, category, and entities',
    tags: ['Signals'],
    security: [{ Bearer: [] }],
    request: {
  ...
  async (c) => {
    ...
    if (!body.source || !body.category || !body.type || !body.impacts?.length) {
      throw ApiError.validation(
        'Source, category, type, and impacts are required',
      );
    }
...
signalsApp.openapi(
  ...
  async (c) => {
    ...
    const invalidIds = Array.from(allEntityIds).filter(
      (id) => !validEntityIds.has(id),
    );
