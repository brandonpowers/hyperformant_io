...
metricsApp.openapi({
  ...
  const [entity, metricDef] = await Promise.all([
    ...
    prisma.metricDefinition.findUnique({ where: { id: body.metricDefinitionId } }),
  ]);
...
metricsApp.openapi({
  ...
  const metricDefIds = [...new Set(body.points.map((p: any) => p.metricDefinitionId))];
  ...
  const invalidMetricDefIds = metricDefIds.filter(id => !validMetricDefIds.has(id));

  if (invalidEntityIds.length > 0) {
    throw ApiError.validation(`Invalid entity IDs: ${invalidEntityIds.join(', ')}`);
  }
  if (invalidMetricDefIds.length > 0) {
    throw ApiError.validation(`Invalid metric definition IDs: ${invalidMetricDefIds.join(', ')}`);
  }
  ...
  return c.json(ApiResponse.created({ created: results.length, points: results }), 201);
});
