...
async function ReportMetrics({ userId }: { userId: string }) {
  const [totalReports, pendingReports, completedReports, recentActivity] =
    await Promise.all([
      prisma.report.count({
        where: { userId },
      }),
      ...
      {metrics.map((metric, index) => (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div
              className={`${metric.color} w-12 h-12 rounded-lg flex items-center justify-center text-white text-xl`}
            >
              {metric.icon}
            </div>
          </div>
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Start by adding a company and generating your first Market Forces
          report.
        </p>
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Please sign in to view your reports dashboard.
          </p>
        </div>
      </div>
    );
